...
class TravelPlannerAgentExecutor(AgentExecutor):
    """
    AgentExecutor for the Travel Planner agent.
    """

    def __init__(self, runner: Runner, event_queue: EventQueue):
        self.runner = runner
    ...
    async def _upsert_session(self, session_id: str):
        session = await self.runner.session_service.get_session(
            app_name=self.runner.app_name, user_id="TravelPlanner", session_id=session_id
        )
        if session is None:
...
def convert_a2a_part_to_genai(part: Part) -> types.Part:
    ...
    if isinstance(root, DataPart):
        # Serialize structured data as a JSON text part so the LLM can process it
        import json
        return types.Part(text=json.dumps(root.data))
    if isinstance(root, FilePart):
        if isinstance(root.file, FileWithUri):
            return types.Part(
                file_data=types.FileData(
                    file_uri=root.file.uri, mime_type=root.file.mime_type
                )
            )
        if isinstance(root.file, FileWithBytes):
            ...
            return types.Part(
                inline_data=types.Blob(
                    ...
                    mime_type=root.file.mime_type or "application/octet-stream",
                )
            )
...
def convert_genai_parts_to_a2a(parts: list[types.Part]) -> list[Part]:
    ...
    return [
        ...
        for part in parts
        if (part.text is not None or part.file_data is not None or part.inline_data is not None)
    ]
...
def convert_genai_part_to_a2a(part: types.Part) -> Part:
    ...
    if part.inline_data is not None:
        ...
        try:
            ...
            import base64
            decoded = base64.b64encode(raw).decode("ascii")
        return Part(
            root=FilePart(
                file=FileWithBytes(
                    bytes=decoded,
